...
def chamfer_distance(p1: torch.Tensor, p2: torch.Tensor) -> torch.Tensor:
    """
    计算两个点云的双向 Chamfer Distance (无须 pytorch3d)
    
    Args:
        p1: [B, N, 3] Source Cloud (e.g., Robot Model)
        p2: [B, M, 3] Target Cloud (e.g., Observed MASt3R)
    
    Returns:
        loss: scalar tensor
    """
    # 扩展维度以利用广播计算距离矩阵
    # 注意：如果点数巨大 (N, M > 5000)，torch.cdist 会非常吃显存。
    # 建议 N, M 控制在 2048 - 4096 左右
    
    # torch.cdist 计算 p1 中每点到 p2 中每点的欧氏距离
    # dist_mat: [B, N, M]
    dist_mat = torch.cdist(p1, p2)
    
    # 1. p1 到 p2 最近邻: min over M (dim=2)
    min_dist_p1, _ = torch.min(dist_mat, dim=2)  # [B, N]
    
    # 2. p2 到 p1 最近邻: min over N (dim=1)
    min_dist_p2, _ = torch.min(dist_mat, dim=1)  # [B, M]
    
    # 双向求和平均
    loss = torch.mean(min_dist_p1) + torch.mean(min_dist_p2)
    return loss
