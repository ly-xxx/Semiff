...
def main():
    parser = argparse.ArgumentParser(description="Build aligned assets with robot")
    parser.add_argument("--urdf", type=str, required=True, help="Path to robot URDF file")
    parser.add_argument("--splat", type=str, default="outputs/splat/scene.ply", help="Path to trained 3DGS PLY file")
    parser.add_argument("--qpos_json", type=str, help="Path to joint pose JSON file (from make_joint_json.py)")
    parser.add_argument("--out_dir", type=str, default="outputs/assets", help="Output directory")
    args = parser.parse_args()

    # === 配置 ===
    scan_ply_path = Path(args.splat)
    ...
    # 设置机器人当前的关节角
    # 优先从JSON文件读取，否则使用零位姿态
    # 获取机器人关节信息
    active_joints = sampler.robot.get_active_joints()
    joint_names = [j.name for j in active_joints]
    num_joints = len(joint_names)
    
    if args.qpos_json and Path(args.qpos_json).exists():
        print(f"Loading joint pose from: {args.qpos_json}")
        with open(args.qpos_json, 'r') as f:
        ...
        for name in joint_names:
            if name in pose_dict:
                qpos.append(pose_dict[name])
            else:
                logger.warning(f"Joint {name} not found in JSON, using 0.0")
                qpos.append(0.0)
        
        qpos = np.array(qpos)
        print(f"Loaded {len(qpos)} joint angles from JSON")
    else:
        # 默认零位姿态
        print(f"Using default zero pose (no JSON provided, {num_joints} joints)")
        qpos = np.zeros(num_joints)
    ...
    try:
        ...
        print(f"   Joint count: {len(qpos)}")
        print("   Try adjusting the joint count or check URDF structure")
        return
    ...
    try:
        ...
        print(f"Coarse Alignment Fitness: {ransac_res.fitness:.4f}")
